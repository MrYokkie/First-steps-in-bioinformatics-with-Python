...
path <- "C:/Users/vredko/Desktop/NGS/Drugi_poziom_fastq"
# check 
list.files(path)
...
REV <- "GACTACACGGGTATCTAATCC" ## reverse primer

# Dapters used by Genomed
# FWD <- "CCTACGGGAGGCAGCAG"
# REV <- "CAGTGCCTTTGCAACTGT"
...
cutadapt <- "C:/Users/vredko/AppData/Local/Packages/PythonSoftwareFoundation.Python.3.10_qbz5n2kfra8p0/LocalCache/local-packages/Python310/site-packages"
...
out <- filterAndTrim(cutFs, filtFs, cutRs, filtRs, maxN = 0, maxEE = 2, minQ = 20,
...
path_new<-"C:/Users/vredko/Desktop/NGS/Drugi_poziom_fastq"
Fs <- sort(list.files(path_new, pattern = "_R1_001.fastq", full.names = TRUE))
Rs <- sort(list.files(path_new, pattern = "_R2_001.fastq", full.names = TRUE))
...
rownames(taxa2) <- paste0("ASV_", seq(nrow(taxa)))

# Save a tab delimited txt-file of the taxa table for phyloseq analysis:
# Remove the k__, p__, etc. prefixes in the taxa table:
taxa2 <- gsub("[A-z]__","",taxa2)
write.table(taxa2,file="tax2_table.txt",sep="\t", quote=F,col.names=NA)
...
# remove columns on ID, sequence, similarity, ref-database, a cession nos (because these are aggregated values now so these columns don't apply anymore)
# to build phylogenetic tree use frameworks ape and phuloseq
...
random_tree = rtree(ntaxa(ps), rooted=TRUE)
plot(random_tree)
